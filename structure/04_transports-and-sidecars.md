...
- 기존 구현 및 제약 조건: Codex emits `web_search_preview`, private search controls, root union schemas, `custom_tool_call` items such as `apply_patch`, and client-executed native `tool_search`; the public API and Grok CLI proxy share the search dialect, while only the CLI proxy requires root-union flattening, both xAI Responses destinations reject the native custom-tool replay shape, and xAI accepts `tool_search.execution` only as `server` or unset.
...
- 선택한 방식: Normalize hosted search on both official xAI Responses hosts, flatten only losslessly representable CLI-proxy root unions under depth/node/variant budgets, lower bare custom tools to functions, flatten Codex `namespace` groups to collision-safe function aliases, omit the client-only execution marker from native `tool_search` declarations on the xAI wire, and restore custom-call plus namespace identity only on the client-facing response branch.
...
`previous_response_id`, the state layer keeps that input instead of prepending the same history again.
Kiro still sends replayed text and tool structure because its wire requires explicit history, but it
omits image bytes from the completed replay prefix. Images in the current user/tool-result suffix —
including an adapter-owned completion retry built from that same request — remain attached. Pixel
inspection is therefore scoped to the turn that introduced an image; a later turn must attach it
