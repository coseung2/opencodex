...
| OAuth | Login/status/logout for OAuth-backed providers, plus multiauth account management: `GET /api/oauth/accounts`, `PUT /api/oauth/accounts/active`, `PUT /api/oauth/accounts/alias`, `PUT /api/oauth/accounts/pause`, and `DELETE /api/oauth/accounts` list masked accounts per provider, switch the active one, edit its display-only alias, pause it, and remove one. The login flow itself is `GET /api/oauth/providers`, `POST /api/oauth/login`, `POST /api/oauth/login/code`, `POST /api/oauth/login/cancel`, `POST /api/oauth/logout`, and `GET /api/oauth/status`; pool controls are `GET/PUT/PATCH /api/oauth/accounts/pool` and `POST /api/oauth/accounts/clear-cooldown`. Anthropic exposes quota-aware strategy, sticky limit, same-request failover limit, and fallback cooldown. Kiro exposes enablement, same-request failover limit, and fallback cooldown for its sticky, error-driven pool because it has no reliable per-account quota endpoint. Login accepts `addAccount: true` to force a fresh browser identity. Kiro organization add-account accepts the transient body field `kiroOrganization: { startUrl, region }` only together with `provider: "kiro"` and `addAccount: true`; it validates and canonicalizes an AWS `*.awsapps.com/start` portal, then runs AWS SSO OIDC device authorization inside opencodex without the local Kiro CLI or its session, so no CLI logout or database snapshot/rollback applies on that path. The verification URL is whatever AWS returns for that portal, commonly `https://<portal>.awsapps.com/start/#/device?user_code=...` rather than a `device.sso` host, so it is surfaced as returned instead of being reconstructed. Organization accounts are keyed by a composite user-plus-organization identity, not by a single identifier. The raw Start URL and other typed input fields are request-scoped and are not persisted, while the resolved AWS region is stored per account as `ssoRegion` so refresh routing reaches the same regional endpoint. Device flows return a structured `deviceCode`; the GUI highlights and copies it before the user opens the verification page. |
