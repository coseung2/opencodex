...
const EXPECTED_KEY_PROVIDER_IDS = [
  ...
  "deepseek", "cerebras", "deepinfra", "hyperbolic", "nscale", "vultr", "baseten", "nebius", "digitalocean", "scaleway", "together", "fireworks", "firepass", "moonshot",
...
describe("provider registry parity", () => {
  ...
  test("key-login export is derived from the registry", () => {
    ...
    expect(KEY_LOGIN_PROVIDERS["openai-apikey"].models).toEqual(["gpt-5.5", "gpt-5.6", "gpt-5.6-sol", "gpt-5.6-terra", "gpt-5.6-luna", "gpt-5.6-sol-pro", "gpt-5.6-terra-pro", "gpt-5.6-luna-pro", "gpt-6-sol", "gpt-6-luna", "gpt-6-astra"]);
    ...
    expect(KEY_LOGIN_PROVIDERS.deepseek.models).toContain("deepseek-v4-flash-vision-exp");
  ...
  test("CN provider defaults and context windows match the audited registry refresh", () => {
    ...
    expect(deepseek).toMatchObject({
      ...
      modelInputModalities: { "deepseek-v4-flash-vision-exp": ["text", "image"] },
    });
  ...
  test("Alibaba Token Plan exposes the official Beijing model contract separately from Coding Plan", () => {
    ...
    expect(KEY_LOGIN_PROVIDERS["alibaba-token-plan"]).toMatchObject({
      ...
      modelReasoningEfforts: {
        ...
        "qwen3.6-flash": ["low", "medium", "high", "xhigh", "max"],
  ...
  test("Z.AI and Kimi context aliases route with bracket-suffix stripping", () => {
    ...
    const optedInProviders = PROVIDER_REGISTRY
      ...
      .map(entry => entry.id);
    expect(zai?.modelContextWindows).toEqual({
      "glm-5.3": 1_000_000, "glm-5.3[1m]": 1_000_000,
