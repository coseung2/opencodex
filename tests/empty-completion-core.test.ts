...
function fixtureAdapter(provider: OcxProviderConfig): ProviderAdapter {
  ...
  return {
    name: provider.terminalContinuationGuard ? "anthropic" : runTurn ? "test-run-turn" : "openai-chat",
    buildRequest(parsed) {
...
function logContext(adapter: string): RequestLogContext {
  return {
    model: "model",
    provider: "fixture",
    activeAttempt: beginRequestAttempt(1, "fixture", "model", adapter),
  };
}
...
describe("empty-completion core integration", () => {
  for (const stream of [true, false]) {
    test(`runTurn ${stream ? "streaming" : "non-streaming"} retries on a fresh queue`, async () => {
      ...
      const logCtx = logContext("test-run-turn");
    ...
    test(`HTTP ${stream ? "streaming" : "non-streaming"} replays the identical request once`, async () => {
      ...
      const logCtx = logContext("openai-chat");
