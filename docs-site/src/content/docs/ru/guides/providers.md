...
Только учётные данные Kimi без идентификатора заменяют активный слот; аккаунты Kiro сохраняются по определённой идентичности аккаунта, а для входа организации это пользователь вместе с организацией.
...
Импортированные учётные данные сохраняются в `~/.opencodex/auth.json`. В **Добавить аккаунт** выбирается личный аккаунт для обычного Kiro Builder ID или аккаунт организации IAM Identity Center. Личный аккаунт по-прежнему работает через локальный `kiro-cli`, а аккаунт организации локальный CLI не использует вовсе.

Для организации укажите Start URL (`https://<portal>.awsapps.com/start`; поддерживаются созданные AWS адреса `d-...` и пользовательские имена порталов) и регион AWS. OpenCodex сам выполняет device authorization по AWS SSO OIDC: регистрирует клиента, запускает авторизацию устройства для этого портала, автоматически открывает в браузере тот URL подтверждения, который вернул AWS, и показывает одноразовый код пользователя в дашборде и в Notch. Этот адрес AWS возвращает для вашего портала, и часто это сам портал доступа (`https://<portal>.awsapps.com/start/#/device?user_code=...`), а не хост `device.sso`, поэтому открывайте ссылку как есть и не рассчитывайте на фиксированный адрес. Подтвердите запрос на этой странице вручную; OpenCodex опрашивает AWS до выдачи токена, определяет идентичность аккаунта и добавляет результат в пул аккаунтов Kiro, используя как ключ пользователя вместе с организацией, а не один отдельный идентификатор. Регион сохраняется вместе с этим аккаунтом как `ssoRegion`, чтобы последующее обновление токена шло к тому же эндпоинту AWS; Start URL и остальные введённые значения используются только для запуска авторизации и не сохраняются. Существующие аккаунты продолжают работать, а при отмене или ошибке пул остаётся без изменений.

Поскольку путь организации не затрагивает Kiro CLI, он работает на машине без установленного `kiro-cli` и не нарушает активную сессию `kiro-cli`.

Дальнейшая работа с сессией Kiro CLI относится только к принудительному входу для **личного аккаунта**. Этот путь делает снимок текущей сессии, выходит из `kiro-cli` и импортирует новый вход как ещё один аккаунт пула Kiro. Откат при восстановлении предыдущего снимка заменяет базу и удаляет текущие sidecar-файлы WAL, SHM и journal.

Поскольку откат возможен только при наличии снимка, путь личного аккаунта откажется выходить из `kiro-cli`, если хранилище сессии существует, но его нельзя захватить (файл не читается, несовпадение схемы, неоднозначный выбор токена), если `KIROCLI_DB_PATH` / `KIRO_CLI_DB_FILE` направляют импорт не на активное хранилище CLI, или если в основной базе CLI нет распознаваемой строки токена. Исправьте или удалите повреждённую базу по обычному пути данных `kiro-cli`, снимите селекторы только для импорта и повторите попытку. На машины без существующей сессии `kiro-cli` это не влияет.
