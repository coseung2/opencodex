...
Когда аккаунт ChatGPT добавляется в пул аккаунтов Codex или проходит повторную аутентификацию, opencodex проверяет новые учётные данные до сохранения
...
`tokenGuardian.codexWarmupEnabled` равен true. Исключение — аккаунт, для которого свежий ответ
usage подтверждает исчерпание квоты: opencodex сохраняет новые или заменённые учётные данные приостановленными в состоянии ожидания
прогрева и автоматически повторяет прогрев после указанного времени сброса. Ошибки аутентификации
и другие ошибки прогрева по-прежнему отклоняют добавление или повторную аутентификацию аккаунта.
