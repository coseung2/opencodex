...
| `autoSwitchThreshold?` | `number` | `80` | Порог проактивного переключения по использованию. `quota` может повторно оценить следующий запрос как привязанной, так и непривязанной задачи; `fill-first` использует его только как точку исчерпания для непривязанных назначений; обычный `round-robin` его не использует. Plus/Team/Business используют доступное окно 5 часов, но недельное или 30-дневное использование на уровне 100% означает исчерпание даже при остатке в 5-часовом окне. Для других планов и неполных данных используется известное недельное или 30-дневное окно, соответствующее плану. `0` отключает только переключение по использованию, но не назначение непривязанных задач и не восстановление после сбоев. |
...
Для добавленных аккаунтов Codex ответ **401** до начала потока сначала вызывает одно обновление токена
и один повтор запроса с тем же аккаунтом. Временная ошибка обновления допускает повторную попытку.
Отозванные или истёкшие данные для обновления, повторный 401 после обновления или **403** сбрасывают
локальную привязку и требуют повторной аутентификации. Данными основного аккаунта по-прежнему управляет приложение Codex.
