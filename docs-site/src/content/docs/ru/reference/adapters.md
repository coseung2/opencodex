...
`metadataEvent` может нести нативный `stopReason`. `END_TURN` и `STOP_SEQUENCE` считаются
авторитетным завершением: их текст выдаётся как финальный ответ без дополнительного обращения к модели.

Путь совместимости используется только при **отсутствии** stop reason. Любая явная причина уже
...
Только при полном отсутствии stop reason opencodex добавляет приватный инструмент
`codex_kiro_final_answer` и делает одно продолжение. Подавление дубликатов ограничено точным
совпадением после нормализации пробелов: переформулированное обновление статуса может изменить исход
хода («ещё выполняется» → «завершено»), и потерять эту фразу хуже, чем показать косметический повтор.
