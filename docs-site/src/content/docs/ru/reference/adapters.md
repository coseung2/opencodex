...
- В локально развёрнутых ходах с `previous_response_id` текст и структура инструментов остаются в
  истории, но байты изображений из уже завершённых предыдущих ходов повторно не отправляются.
  Изображения из текущего user/tool-result suffix и его ограниченной попытки завершения сохраняются.
  Чтобы снова проверить пиксели в более позднем ходе, прикрепите изображение повторно.
