...
| **ログ** | トークン、出力 tok/s、要求された強度と（利用可能な場合は）実際に送信された強度、実際のモデル、プロバイダー、状態、リクエスト ID、エラー詳細を含む最近のリクエストを自動更新します。所要時間は引き続きリクエスト詳細で確認でき、アダプターが reasoning パラメーターを送信した場合は正確な wire field も表示されます。 |
...
- 新規セッションは使用量が最も低い健全アカウントを選べます。5 時間枠を提供するプラン
  （現在は Plus/Team/Business）は 5 時間値があればその枠をローテーション基準に使い、まだなければ既知の
  週間または 30 日枠にフォールバックします。Pro/ProLite は提供される枠を使い、Go/Free プランは 30 日枠のみ使います。
- WHAM が `limit_window_seconds` を提供すると、Codex Auth は 28 日以上の primary window を週次ではなく
  30 日枠として分類します。単一枠で期間がない場合は従来どおり週次として扱い、Plus/Team/Business の
  2 枠レスポンスでは従来の primary=5 時間、secondary=週次の順序を維持します。
