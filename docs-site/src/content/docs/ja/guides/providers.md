...
ロールバックはスナップショットがある場合にのみ可能なため、セッションストアが存在するのに取得できない場合（ファイルが読めない、スキーマの不一致、トークン選択があいまい）、**アカウントを追加**は `kiro-cli` のログアウトを拒否します。まずローカルストアを解決してから再試行してください。既存の `kiro-cli` セッションがまったくない環境には影響しません。
