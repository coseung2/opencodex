...
アカウント識別情報がない Kimi 認証情報だけがアクティブスロットを差し替え、Kiro アカウントは解決されたアカウント識別情報をキーに保存され、組織ログインではユーザーと組織の組み合わせを使います。
...
取り込んだ認証情報は `~/.opencodex/auth.json` に保存されます。**アカウントを追加**では、通常の Kiro Builder ID 用の**個人アカウント**か、IAM Identity Center 用の**組織アカウント**を選びます。個人アカウントは従来どおりローカルの `kiro-cli` を使い、組織アカウントはローカル CLI を一切使いません。

組織アカウントには Start URL（`https://<portal>.awsapps.com/start`。AWS 生成の `d-...` とカスタムポータル名に対応）と AWS リージョンを入力します。OpenCodex が AWS SSO OIDC のデバイス認証を自ら実行します。クライアントを登録してそのポータルに対してデバイス認証を開始し、AWS が返した確認 URL をブラウザーで自動的に開き、ワンタイムのユーザーコードをダッシュボードと Notch に表示します。この URL は AWS がそのポータルに対して返すもので、`device.sso` ホストではなくアクセスポータル自体（`https://<portal>.awsapps.com/start/#/device?user_code=...`）であることが多いため、固定のアドレスを想定せず開かれたリンクをそのまま使ってください。そのページでコードを確認して承認すると、OpenCodex がトークン発行までポーリングし、アカウント識別情報を解決して Kiro プールアカウントとして追加します。キーは単一の識別子ではなく、ユーザーと組織の組み合わせです。リージョンはその後のトークン更新が同じ AWS エンドポイントに向かうよう、そのアカウントの `ssoRegion` として保存されます。Start URL など入力した他の値は認証リクエストの開始にのみ使用され、保存されません。既存のアカウントはそのまま動作し、キャンセルや失敗の場合はプールは変更されません。

組織アカウントの経路は Kiro CLI に触れないため、`kiro-cli` が未インストールの環境でも動作し、ログイン済みの `kiro-cli` セッションにも影響しません。

以下の Kiro CLI セッション処理は、強制的な新規ログインを行う**個人アカウント**の経路にのみ適用されます。この経路は現在のセッションをスナップショットし、`kiro-cli` をログアウトして、新しいログインを Kiro プールアカウントとして取り込みます。ロールバックは以前のスナップショットを復元する際にデータベースを置き換え、現在の WAL、SHM、journal サイドカーを削除します。

ロールバックはスナップショットがある場合にのみ可能なため、セッションストアが存在するのに取得できない場合（ファイルが読めない、スキーマの不一致、トークン選択があいまい）、`KIROCLI_DB_PATH` / `KIRO_CLI_DB_FILE` が実際の CLI ストアと異なるインポート先を指す場合、またはプライマリ CLI データベースに認識できるトークン行がない場合、個人アカウントの経路は `kiro-cli` のログアウトを拒否します。通常の `kiro-cli` データパス上の壊れたデータベースを修復または削除し、インポート専用セレクタが設定されていれば解除してから再試行してください。既存の `kiro-cli` セッションがまったくない環境には影響しません。
