...
| `autoSwitchThreshold?` | `number` | `80` | 使用量ベースのプロアクティブ切り替えしきい値。`quota` は紐付け済み/未紐付けタスクの次のリクエストを再評価でき、`fill-first` は未紐付け割り当ての使い切り基準としてのみ使用し、通常の `round-robin` 選択は使用しません。Plus/Team/Business は 5 時間の使用率を優先しますが、週次または 30 日の使用率が 100% なら、5 時間枠に余裕があっても上限到達と判断します。他のプランや一部のデータしかない場合は、そのプランに該当する既知の週次または 30 日の使用率を使います。`0` は使用量ベースの切り替えだけを無効にし、未紐付け割り当てや障害回復は無効にしません。 |
...
追加した Codex アカウントでストリーミング開始前に **401** が返ると、トークンを一度更新し、同じアカウントで一度再試行します。
一時的な更新失敗は再試行可能な状態を維持します。更新資格情報の失効・期限切れ、更新後も続く 401、または **403** では、
そのアカウントへのプロセスローカルな affinity を解除し、再認証を要求します。メインアカウントの資格情報は Codex アプリが管理します。
