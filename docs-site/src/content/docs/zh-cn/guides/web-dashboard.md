...
| **Logs** | 自动刷新近期请求，显示 token、输出 tok/s、请求强度以及（可用时）实际发送强度、实际模型、provider、状态、request id 和错误详情。耗时仍可在请求详情中查看；适配器发送 reasoning 参数时，详情中还会显示准确的 wire field。可按不透明会话/对话 ID（客户端提供时）筛选，并对当前已加载的 Logs 环形缓冲合计 token 与估算标价成本。 |
