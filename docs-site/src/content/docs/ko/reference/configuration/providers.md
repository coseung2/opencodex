...
| `autoSwitchThreshold?` | `number` | `80` | 사용량 기반 선제 전환 임계값입니다. `quota`는 바인딩된 작업과 바인딩 없는 작업의 다음 요청을 모두 재평가할 수 있고, `fill-first`는 바인딩 없는 작업 배정의 소진 기준으로만 사용하며, 기본 `round-robin` 선택은 이 값을 사용하지 않습니다. Plus/Team/Business는 5시간 사용률을 우선하지만, 주간이나 30일 사용률이 100%이면 5시간 잔여량과 관계없이 소진으로 판단합니다. 다른 요금제나 일부 데이터만 있는 경우에는 해당 요금제의 알려진 주간 또는 30일 사용률을 씁니다. `0`은 사용량 기반 전환만 끄며 바인딩 없는 작업 배정이나 실패 복구는 끄지 않습니다. |
...
공식 OpenCode Go 엔드포인트에 `apiKeyPool` 키가 2개 이상이면 Responses 요청(변환된 Chat 요청 포함)을
보내기 전에 활성 키의 할당량을 확인합니다. 5시간·주간·월간 한도 중 하나가 100%이면 다음 적격 키로
전환합니다. 실제 사용량 조회 결과는 15초 동안 캐시하며, 조회 실패나 로컬 추정치만으로 키를 제외하지
않습니다. 요청당 서로 다른 키 8개까지 조회하고, 그 이후 미조회 키는 일반 오류 복구 대상으로 남깁니다.

Chat 전송과 Muse를 포함한 네이티브 Responses 전송 모두 스트리밍 시작 전 429에서도 키를 전환합니다.
재시도는 요청 내용과 세션 메타데이터를 보존하고, 이미 소진 또는 쿨다운으로 확인된 키는 건너뜁니다.
사전 조회에서 모든 키가 막혀 있으면 `Retry-After`가 포함된 429를 반환합니다. 한도 리셋 시각이 지나면
다시 선택 대상이 될 수 있습니다. 이미 전달한 스트리밍 출력은 재전송하지 않습니다.
전환으로 서비스를 계속 이용하려면 다음 키에 독립적인 잔여 할당량이 있어야 합니다.
...
추가한 Codex 계정에서 스트리밍 시작 전에 **401**이 발생하면 토큰을 한 번 갱신하고 같은 계정으로 한 번 재시도합니다.
일시적인 갱신 실패는 재시도 가능한 상태로 유지합니다. 갱신 자격 증명이 폐기되거나 만료된 경우,
갱신 후에도 401인 경우, 또는 **403**인 경우에는 해당 계정의 프로세스 로컬 affinity를 해제하고 재인증을 요구합니다.
메인 계정의 자격 증명은 Codex 앱이 계속 관리합니다.
...
Plus/Team/Business는 주간 사용량이나 크레딧만 갱신돼도 기존 5시간 사용량과 리셋 시각을 보존합니다.
유효한 5시간 사용량이 100%이면 주간 정보가 없어도 전환할 수 있습니다. 리셋 시각이 지나면 알려진
장기 사용량으로 판단하며, 리셋 시각이 없는 새 관측값은 5분 동안 사용합니다. 부분 갱신은 이 시간을
연장하지 않습니다. Pro와 Pro Lite는 계속 장기 한도 기준으로 판단합니다.
