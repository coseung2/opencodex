...
Plus/Team/Business는 가능한 경우 5시간 사용률을 선제 전환 기준으로 사용합니다.
주간이나 월간 사용률이 100%이면 5시간 사용률이 낮아도 소진으로 판단합니다.
할당량 기반 전환이 켜져 있으면 기존 대화의 다음 요청은 잔여량이 있는 적격 계정으로 이동합니다.
재인증이 필요한 계정은 제외합니다. 스트리밍 중 `response.incomplete`에 할당량 오류가 포함된 경우에도
해당 계정을 대기 상태로 바꿔 다음 요청에서 전환할 수 있도록 합니다. 이미 시작한 스트림은 재실행하지 않으며,
일반적인 출력 토큰 한도는 계정 대기 상태를 만들지 않습니다.

### `ocx account login|reauth|code|cancel ...`

헤드리스 셸에서 브라우저 기반 또는 수동 코드 계정 인증을 실행합니다. 제공자별 명령 형태는 `ocx account --help`를 보십시오.

추가한 Codex 계정이 스트리밍 시작 전에 HTTP 401을 반환하면 저장된 만료 시각이 남아 있어도
토큰을 한 번 자동 갱신하고 같은 계정으로 한 번 재시도합니다. 일시적인 갱신 실패는 재시도할 수 있습니다.
갱신 자격 증명이 폐기되거나 만료된 경우, 또는 갱신 후에도 401인 경우에만 재인증을 요구합니다.
