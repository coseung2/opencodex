...
계정 식별 정보가 없는 Kimi 자격 증명만 활성 슬롯을 교체하며, Kiro 계정은 확인된 계정 식별 정보를 키로 저장되며, 조직 로그인에서는 사용자와 조직의 조합을 사용합니다.
...
가져온 자격 증명은 `~/.opencodex/auth.json`에 저장됩니다. **계정 추가**에서는 일반 Kiro Builder ID용 **개인 계정**과 IAM Identity Center용 **조직 계정** 중 하나를 선택합니다. 개인 계정은 기존처럼 로컬 `kiro-cli`를 사용하고, 조직 계정은 로컬 CLI를 전혀 사용하지 않습니다.

조직 계정에는 Start URL(`https://<portal>.awsapps.com/start`, AWS가 만든 `d-...` 주소와 사용자 지정 포털 이름 모두 지원)과 AWS 리전을 입력합니다. OpenCodex가 AWS SSO OIDC 디바이스 인증을 직접 수행합니다. 클라이언트를 등록하고 해당 포털에 디바이스 인증을 시작한 뒤 AWS가 돌려준 확인 URL을 브라우저에서 자동으로 열고, 대시보드와 Notch에 일회용 사용자 코드를 표시합니다. 이 URL은 AWS가 해당 포털에 대해 반환하는 주소이며 `device.sso` 호스트가 아니라 액세스 포털 자체(`https://<portal>.awsapps.com/start/#/device?user_code=...`)인 경우가 많으므로, 고정된 주소를 기대하지 말고 열린 링크를 그대로 사용하세요. 그 페이지에서 코드를 확인해 직접 승인하면 OpenCodex가 토큰 발급까지 폴링하고 계정 식별 정보를 확인해 Kiro 풀 계정으로 추가하며, 단일 식별자 하나가 아니라 사용자와 조직을 함께 키로 사용합니다. 리전은 이후 토큰 갱신이 같은 AWS 엔드포인트로 가도록 해당 계정의 `ssoRegion`으로 함께 저장되며, Start URL 등 입력한 나머지 값은 인증 요청 시작에만 사용하고 저장하지 않습니다. 기존 계정은 그대로 유지됩니다. 취소하거나 실패하면 풀은 변경되지 않습니다.

조직 계정 경로는 Kiro CLI를 건드리지 않으므로 `kiro-cli`가 설치되지 않은 컴퓨터에서도 동작하고, 로그인된 `kiro-cli` 세션에도 영향을 주지 않습니다.

아래 Kiro CLI 세션 처리는 강제 새 로그인을 수행하는 **개인 계정** 경로에만 적용됩니다. 이 경로는 현재 세션을 스냅샷으로 저장하고 `kiro-cli`를 로그아웃한 뒤 새 로그인을 Kiro 풀 계정으로 가져옵니다. 롤백은 이전 스냅샷을 복원할 때 데이터베이스를 교체하고 현재 WAL, SHM, journal 사이드카를 제거합니다.

롤백은 스냅샷이 있을 때만 가능하므로, 세션 저장소가 존재하지만 캡처할 수 없는 경우(파일을 읽을 수 없음, 스키마 불일치, 토큰 선택 모호), `KIROCLI_DB_PATH` / `KIRO_CLI_DB_FILE`이 실제 CLI 저장소와 다른 가져오기 경로를 가리키는 경우, 또는 기본 CLI 데이터베이스에 인식 가능한 토큰 행이 없는 경우 개인 계정 경로는 `kiro-cli` 로그아웃을 거부합니다. 일반 `kiro-cli` 데이터 경로의 손상된 데이터베이스를 수리하거나 제거하고, 가져오기 전용 선택자가 설정돼 있으면 해제한 뒤 다시 시도하세요. 기존 `kiro-cli` 세션이 아예 없는 환경에서는 영향이 없습니다.
