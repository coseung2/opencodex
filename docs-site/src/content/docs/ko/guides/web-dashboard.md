...
| **Logs** | 토큰, 출력 tok/s, 요청한 강도와 (사용 가능한 경우) 실제 전송 강도, 실제 모델, 프로바이더, 상태, 요청 id, 오류 상세가 포함된 최근 요청을 자동 갱신합니다. 소요 시간은 요청 상세에서 계속 확인할 수 있으며, 어댑터가 reasoning 매개변수를 전송한 경우 정확한 wire field도 표시됩니다. 클라이언트가 보낸 불투명 대화/세션 id로 필터하면 현재 로드된 Logs 링의 토큰·추정 정가 합계를 볼 수 있습니다. |
