...
전체 대화를 직접 보내는 클라이언트에서도 도구 작업을 이어갈 때 최신 assistant 응답 이전의 이미지 바이트는 다시 보내지 않습니다. 새 도구 결과 이미지는 유지하며, 사용자가 새로 말하는 턴에서는 가장 최근 이미지가 포함된 메시지를 참조할 수 있습니다. 진행 설명과 도구 결과는 보존하고, 설명 반복만을 이유로 작업을 강제로 종료하지 않습니다.
