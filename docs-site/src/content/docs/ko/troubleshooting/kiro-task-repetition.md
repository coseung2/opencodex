...
회귀 테스트는 빈 결과 설명 누락과 도구 목록 탈락을 재현하지만 모든 모델 반복 현상의 원인이 같다는 뜻은 아닙니다. 수정 빌드 배포 후에도 반복된다면 직전 결과가 빈 출력인지, 오류인지, 정상 결과인지, 이미 전달한 최종 답변인지 구분해야 합니다. 실행 중인 버전과 민감 정보를 제거한 요청 식별자를 기록하되 자격 증명이나 원문 비공개 프롬프트를 공개하지 않습니다.
