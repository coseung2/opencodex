...
import { createKiroToolNameRegistry, fallbackToolUseId, isValidKiroConversationId, mapModelId, normalizeToolId, stableConversationId } from "./kiro-wire";
import { namespacedToolName } from "../types";
import {
  boundedKiroInjectedInstruction,
  kiroCompletionTool,
  resolveKiroCompletionMode,
} from "./kiro-continuity";
export { boundedInjectedInstructionForTests } from "./kiro-continuity";
import {
...
export function buildKiroPayload(
  ...
  const completionMode = resolveKiroCompletionMode(parsed, ordinaryTools.length, forcedCompletionMode);
  ...
  for (const addition of toolContext.systemAdditions) {
    const boundedAddition = boundedKiroInjectedInstruction(addition, injectedChars);
  ...
  const boundedNudge = toolCatalogNudge ? boundedKiroInjectedInstruction(toolCatalogNudge, injectedChars) : undefined;
  if (boundedNudge) systemParts.push(boundedNudge);
  if (completionMode !== "disabled") {
    const boundedCompletion = boundedKiroInjectedInstruction(KIRO_COMPLETION_INSTRUCTIONS, injectedChars);
  ...
  for (let messageIndex = 0; messageIndex < payloadMessages.length; messageIndex++) {
    ...
    if (msg.role === "user" || msg.role === "developer") {
      ...
      const rawText = (aMsg.content || [])
        ...
        .map(b => b.text)
        .join("");
      // Responses commentary is already-visible UI progress, but it can also carry durable task
      // state: decisions, completed steps, rejected hypotheses, and the next action. Preserve it in
      // Kiro's explicit history so tool-result continuations and compaction can resume from that
      // state. Historical input is never re-emitted by the stream parser; the completion contract
      // separately tells Kiro not to repeat or paraphrase an earlier progress update.
      const text = rawText;
...
async function* parseKiroAttemptEvents(
  ...
  const classifiedTerminal = (failure: KiroErrorClassification): AdapterEvent => {
    ...
    if (failure.status === 429 && failure.retryable) onTransientThrottle?.();
    return {
      type: "error",
