...
export const POOL_KEY_KIRO = "kiro";
...
export function reconcilePoolRotationState(context: GenerationContext): number {
  ...
  for (const key of context.oauthAccountKeys) {
    ...
    } else if (separator > 0 && key.slice(0, separator) === "kiro") {
      kiroIds.add(key.slice(separator + 1));
    }
  }
  let removed = 0;
